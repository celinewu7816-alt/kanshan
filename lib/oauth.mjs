...
// 原脚手架硬编码 '/usr/bin/curl'（macOS 专用）。改成跨平台；
// Windows 上走 System32\curl.exe（Win10 1803+ 自带）。
const CURL_BIN = process.platform === 'win32' ? 'curl.exe' : '/usr/bin/curl';

function runCurl(lines) {
  return new Promise((resolve, reject) => {
    const child = spawn(CURL_BIN, ['--config', '-'], { stdio: ['pipe', 'pipe', 'pipe'] });
    const stdout = [];
    const stderr = [];

    // 原脚手架没有监听 'error'。spawn 失败（例如可执行文件不存在）时，
    // Node 会把未处理的 'error' 当成未捕获异常 → 整个进程挂掉。
    // 容器里少一个 curl 就能让线上服务一点「授权」就 502，所以必须兜住。
    child.on('error', (error) => {
      reject(new Error(`无法调用 ${CURL_BIN}：${error.message}`));
    });
    ...
    child.stdin.on('error', () => { /* spawn 已失败时写入会 EPIPE，忽略即可 */ });
...
function userRequestConfig(accessSecret, oauthToken, url) {
  return [
    'silent', 'show-error', 'max-time = 8', 'request = "GET"', `url = "${url}"`,
...
export function createOAuth(config) {
  ...
  // 允许用环境变量覆盖回调地址：部署拿到公网域名后，只需在平台上改配置，
  // 不必改文件再重新部署一轮。变量名沿用官方文档里的 ZHIHU_OAUTH_REDIRECT_URI。
  const oauthConfig = { ...config.oauth };
  if (process.env.ZHIHU_OAUTH_REDIRECT_URI) {
    oauthConfig.redirectUri = process.env.ZHIHU_OAUTH_REDIRECT_URI;
  }
  ...
  async function callback(request, response, url) {
    ...
    const payload = await runCurl([
      'silent', 'show-error', 'max-time = 8', 'request = "POST"',
